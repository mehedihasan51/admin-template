    ...
    <div class="p-5">
        ...
        <form method="POST" action="{{ route('system.update') }}" enctype="multipart/form-data">
            ...
            <div class="row g-3 bg-white p-2 box-show">
                ...
                <!-- About System -->
                <div class="col-12">
                    <label for="about" class="form-label">About System</label>
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="About System...">{{ old('description', $setting->description ?? '') }}</textarea>
                    @error('description')
                        <span class="text-danger">{{ $message }}</span>
                        @enderror
                </div>
